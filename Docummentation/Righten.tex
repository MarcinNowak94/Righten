...
{Rozdział ten zawiera dokumentację techniczną projektu. Dokumentacja jest 
...
%#TODO: describe expenditures table - displaying ExpendituresEnriched view while editting underlying data 
...
zapewnić możliwie szybkie postępy prac w kierunku implementacji zaprojektowanych
 funkcji zastosowano najprostszą metodę testów którą są testy manualne. 
Testowaniu podlegały zarówno pojedyncze funkcje, moduły jak i aplikacja jako 
...
\medskip
{W warstwie backend przeprowadzano głównie manualne testy akceptacyjne dla 
każdej implementowanej funkcji aby potwierdzić czy działa zgodnie z założeniami 
i wstępnym projektem. Test każdej wdrożonej funkcji wymagał uruchomienia 
...
{Po dodaniu panelu logowania weryfikacja obejmowała probę wejścia do panelu 
wizualizacji bez logowania, próby wprowadzenia błędnych poświadczeń (tak nazwy 
użytkownika jak i hasła) i weryfikację czy zalogowany użytkownik moze prawidłowo
 poruszać się po aplikacji.}
{Test po dodaniu zabezpieczenia sesji użytkownika opisano w kolejnej sekcji jako
 ciekawy przypadek.}

\medskip
{Warstwa interfejsku użytkownika podlegała głównie manualnych testach 
...
ułożenie elementów odbiegało od założeń projektowych, wybrane elementy 
...
mieściły się w wyznaczonym obszarze. Ich naprawa natomiast sprowadzała sie do 
...
{W miarę rozwoju projektu podczas implementacji poszczególnych funkcji wykryto i
 wyeliminowano wiele błędów, niektóe jednak wyróżniają się od reszty na kilka 
...
nie widoczne na pierwszy rzut oka. Za przykłąd posłuży obliczanie miesięcznej 
średniej dziennych wydatków. Ponieważ zapytanie budowane było etapami w których 
stopniowo rozbudowano je o dodatkowe informacje, jego logika była dość zawiła. 
Na kolejnych fazach projektu podczas usprawnień warstwy bazy danych zapytanie 
zostało zoptymalizowane - części wspólne przeniesiono do osobnego podzapytania 
zbierającego dane dzienne, z któego następnie obliczano dane miesięczne. Efektem
 prac był kod widoczny na listingu \ref{lst:dbmonthlyspending-errror}, który 
mimo iż na pierwszy rzut oka wygląda prawidłowo oblicza nie średnie miesieczne a
 średnie ze średnich dziennych. Błąd logiki wykryto podczas ręcznej weryfikacji 
danych testowych, a logika po naprawie błędu (i zmianie dialektu SQL) widoczna 
jest na listingu \ref{lst:dbmonthlyspending-fixed}.}
...
\begin{minipage}{\textwidth}
    \begin{lstlisting}[ caption={Widok MonthlySpending - poprawiona średnia średnich dziennych (dialekt PostgreSQL)},
        language=SQL,
        deletekeywords={IDENTITY},
        deletekeywords={[2]INT},
        morekeywords={clustered},
...
{Z ciekawszych problemów które wykryto w warstwie backend na uwagę zasługuje 
...
{Funkcja ta wykorzystuje sól zapisaną na początku docelowego skrótu, dodając go 
do testowanego hasła, hashując je i porównując ciągi - w wyniku zwraca jedynie 
informajcę czy hash jest prawidłowy.}
\end{minipage}

%gets mushed with listing
\bigskip
{W warstwie prezentacji danych natomiast ciekawy problem wystąpił podczas 
poprawy zabezpieczeń w aplikacji. Po dodaniu tokenu CSRF (Cross-site Request 
Forgery) aplikacja straciła dostęp do bazy danych. Ponieważ dane wymagane do 
połączenia aplikacji z bazą danych oraz instancja silnika który utrzymuje 
połączenie i wykonuje zapytania jest częścią konfiguracji wstępnej aplikacji, 
zabezpieczenie sesji w ramach aplikacji z wykorzystaniem klucza szyfrowania 
spowodowało blokowanie wykonywanych poza sesją prób połączenia z bazą funkcji 
która mapowała model bazy danych na obiekty wykorzystywane w aplikacji 
działającej w osobnym module models.py. Rozwiązanie problemu wymagało importu we
 wskazanym module obiektu app reprezentującego aplikację w pamięci i dodanania 
polecenia with app.context które wykonuje polecenia zawarte w bloku kodu w 
ramach sesji aplikacji. Problematyczny fragment modułu wraz z opisanymi zmianami
 zaprezentowano na listingu \ref{lst:models-kontekstapplikacji}.}
...
with app.app_context():
    ...
    ProductTypes=Base.classes.ProductTypes
    #(...)\end{lstlisting}
{Wprowadzenie zabezpieczenia CSRF wymagało zmiany sposobu mapowania obiektów 
bazy danych na obiekty aplikacji.}
\end{minipage}
